/**
 * SI Testing Module - Screen 4: Results
 *
 * Public page (no authentication required)
 * Displays completion message and SI score
 *
 * @route /testing/si/result
...
function SITestingResultPage() {
  ...
  useEffect(() => {
    if (!participantName || score === undefined) {
      navigate('/testing/si', { replace: true });
    }
    ...
    <div className="ei-testing-container">
      <div className="card result-display">
        ...
        <div className={`score-display score-${scoreCategory}`}>
          <div className="score-label">Ваш балл социального интеллекта:</div>
        ...
        <div className="score-interpretation">
          {score >= 80 && (
            <p>Отличный результат! Высокий уровень социального интеллекта.</p>
          )}
          {score >= 60 && score < 80 && (
            <p>Хороший результат! Развитый социальный интеллект.</p>
          )}
          {score >= 40 && score < 60 && (
            <p>Средний уровень социального интеллекта.</p>
          )}
          {score < 40 && (
            <p>Есть потенциал для развития социального интеллекта.</p>
          )}
        </div>
...
export default SITestingResultPage;
